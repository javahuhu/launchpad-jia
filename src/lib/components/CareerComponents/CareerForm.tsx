...
export default function CareerForm({ career, formType, setShowEditModal, onCreated, }: { career?: any, formType: string, setShowEditModal?: (show: boolean) => void, onCreated?: (id: string) => void; }) {
  ...
  const saveCareer = async (status: string) => {
    ...
    if (!savingCareerRef.current) {
      ...
      try {
        const response = await axios.post("/api/add-career", careerPayload);

        // guard: response data shape from your add-career route:
        // { message, career: careerDoc, insertedId }
        const data = response?.data ?? {};
        const insertedId =
          (data.insertedId && String(data.insertedId)) ||
          (data.career && (data.career._id || data.career.id)) ||
          undefined;
        ...
        // If parent gave us an onCreated callback, call it and DO NOT redirect.
        // Parent (multi-step form) will advance step and keep working with the existing career.
        if (onCreated && insertedId) {
          // give parent the Mongo _id (string)
          onCreated(String(insertedId));
        } else {
          // fallback: original behavior when used standalone — redirect to listing
          setTimeout(() => {
